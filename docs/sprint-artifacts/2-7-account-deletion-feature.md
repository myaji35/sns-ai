...
### AC1: 계정 삭제 버튼
- [x] 프로필 페이지(`/profile`)에 계정 삭제 버튼 표시
- [x] 버튼 위치: 프로필 정보 하단
- [x] 버튼 색상: 위험(빨강색) 표시
...
- [x] 경고 메시지: "계정을 삭제하면 모든 데이터가 삭제됩니다. 되돌릴 수 없습니다."
- [x] 사용자 이메일 표시
- [x] 실제 이메일 입력으로 재확인
  - [x] 입력 필드: "계정 삭제를 확인하려면 이메일을 입력하세요"
  - [x] 이메일이 일치해야 삭제 버튼 활성화
- [x] 취소 버튼
...
### AC4: 데이터 삭제
- [x] Supabase `auth.users` 테이블에서 사용자 삭제
- [x] `profiles` 테이블에서 사용자 정보 삭제
- [ ] `user_sessions` 테이블(있는 경우) 삭제
- [x] Supabase Storage에서 프로필 사진 삭제
- [x] 관련된 모든 데이터 삭제 (향후 콘텐츠, 설정 등)

### AC5: 데이터 보관 정책
- [x] 소프트 삭제 고려: `deleted_at` 타임스탬프 추가
- [ ] 복구 기능 검토 (선택사항)
- [x] GDPR 규정 준수

### AC6: 에러 처리
- [x] 이미 삭제된 계정: "이미 삭제된 계정입니다"
- [x] 삭제 실패: "계정 삭제에 실패했습니다. 나중에 다시 시도해주세요"
- [x] 네트워크 에러: "연결 실패. 다시 시도해주세요"
- [x] 인증 토큰 만료: "세션이 만료되었습니다. 다시 로그인하세요"

### AC7: 사용자 경험
- [x] 모바일 반응형 디자인
- [x] 로딩 상태 표시
- [x] 삭제 완료 후 로그인 페이지로 리다이렉트
- [x] 성공 메시지: "계정이 삭제되었습니다"

### AC8: 보안
- [ ] 비밀번호 입력으로 추가 보안 (선택사항)
- [x] 계정 삭제 이력 로깅
- [x] 민감한 작업이므로 HTTPS만 허용
...
### Task 1: 계정 삭제 버튼 추가
- [x] `/profile/page.tsx`에 삭제 버튼 추가
  - [x] 버튼 스타일 (위험색)
  - [x] 삭제 모달 트리거

### Task 2: 계정 삭제 확인 모달
- [x] `AccountDeletionModal.tsx` 컴포넌트
  - [x] 경고 메시지 표시
  - [x] 이메일 입력 필드
  - [x] 이메일 유효성 검사
  - [x] 취소/확인 버튼
...
### Task 3: 계정 삭제 API 함수
- [x] `lib/api/auth-api.ts` 확장
  - [x] `deleteAccount()` - 삭제 요청
...
### Task 5: 백엔드 라우트 (선택사항)
- [x] `/api/auth/delete-account` POST 라우트
  - [x] 사용자 인증 확인
  - [ ] 이메일 인증 검증
  - [x] 데이터 삭제 처리
...
### Task 7: 마이그레이션 (선택사항)
- [x] `profiles` 테이블에 `deleted_at` 컬럼 추가
  ```sql
...
#### Files Analyzed (Already Implemented)
- `/apps/web/src/components/profile/AccountDeletionModal.tsx` - 계정 삭제 모달 컴포넌트
- `/apps/web/src/app/api/auth/delete-account/route.ts` - 계정 삭제 API 엔드포인트
- `/apps/web/src/lib/api/auth-api.ts` - deleteAccount() 함수
- `/apps/web/src/__tests__/account-deletion.test.ts` - 계정 삭제 테스트
- `/supabase/migrations/20251115000001_add_deleted_at_to_profiles.sql` - 소프트 삭제 마이그레이션
- `/apps/web/src/components/profile/ProfileDisplay.tsx` - 계정 삭제 버튼 통합

#### Key Features Implemented
- ✅ 이메일 재확인을 통한 2단계 보안
- ✅ 모달 경고 메시지와 데이터 삭제 목록 표시
- ✅ Supabase Storage 프로필 이미지 자동 삭제
- ✅ Hard delete (기본) 및 Soft delete (옵션) 지원
- ✅ HTTPS 강제 (production)
- ✅ 계정 삭제 이력 로깅
- ✅ 한글 에러 메시지
- ✅ 모바일 반응형 디자인
...
### Debug Log
- 모든 핵심 기능이 이미 구현되어 있음을 확인
- 이메일 인증 링크 방식은 선택사항으로 미구현 (현재 이메일 재입력 방식 사용)
- 설정 페이지는 아직 없어 계정 삭제 옵션 미추가
- user_sessions 테이블은 현재 사용하지 않음
...
| 2025-11-15 | Story 구현 완료 확인 - 이미 모든 기능이 구현됨 | Claude Code |
...
**Ready for Review:** All core features implemented
